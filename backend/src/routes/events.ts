...
// @desc    Get all events with filtering (date range or single date, search, category, pagination)
// @access  Public
router.get(
  '/',
  [
    query('category')
      .optional()
      .isIn(['academic', 'social', 'sports', 'cultural', 'career', 'volunteer', 'other']),
    query('date').optional().isISO8601(),
    query('start').optional().isISO8601(), // <-- ajouté
    query('end').optional().isISO8601(),   // <-- ajouté
  ...
  async (req: express.Request, res: express.Response) => {
    try {
      ...
      // 1) Plage start/end si fournie
      if (start || end) {
        const startDate = start ? new Date(start) : new Date();
        const endDate = end ? new Date(end) : new Date(startDate);
        startDate.setHours(0, 0, 0, 0);
        endDate.setHours(0, 0, 0, 0);
        endDate.setDate(endDate.getDate() + 1); // end exclusif => inclure toute la journée "end"
        filter.date = { $gte: startDate, $lt: endDate };
      }
      // 2) Un seul jour via ?date
      else if (date) {
      ...
      // 3) Par défaut: seulement les événements futurs
      else {
